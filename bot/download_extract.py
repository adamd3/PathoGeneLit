# This script is adapted from the `download_extract.py` script from rummagene bot
# with modifications to retrieve prokaryotic gene lists
# Original: https://github.com/MaayanLab/rummagene/blob/main/bot/download_extract.py
...
soffice = shutil.which(
    "soffice", path=":".join(filter(None, [os.environ.get("PATH"), "/Applications/LibreOffice.app/Contents/MacOS/"]))
)
...
def read_doc_as_docx(fr):
    ...
    with tempfile.TemporaryDirectory(prefix="pathogl-") as tmpdir:
        tmpdir = Path(tmpdir)
...
def _read_xml_mentions(root, ref: str):
    ...
    try:
        ...
        return match_parens(
            " ".join(re.split(r"\s+", mention[:indexOfRef])[-15:])
            + "**"
            + " ".join(re.split(r"\s+", xrefNodeText))
            + "**"
        )
...
def _read_xml_supplement(tar: tarfile.TarFile, root: ET.Element):
    members = {
        member_name.name: (member_name, member)
        for member in tar.getmembers()
        for member_name in (PurePosixPath(member.name),)
    }
    for supplementary_material in root.findall(".//supplementary-material"):
        ...
        for media in supplementary_material.findall("./media"):
            ...
            if media_gene_sets:
                ...
                gene_sets += [
                    (f"{member_name.parent.name}-{member_name.name}-{term}", caption, gene_set)
                    for term, gene_set in media_gene_sets
                ]
        #
        if gene_sets:
            # given that we have genesets, assemble description and yield them
...
def extract_gmt_from_oa_package(oa_package):
    ...
    with tarfile.open(oa_package) as tar:
        for member in tar.getmembers():
            ...
            if member_path.suffix.lower() not in (".nxml", ".xml"):
                continue
...
def extract_gene_set_columns(df):
    ...
    for col in df.columns:
        ...
        if unique_genes.shape[0] >= 5:
            ...
            if ratio > 0.5:
                yield col, unique_genes.apply(
                    lambda gene: re.sub(r"\s+", " ", gene) if type(gene) == str else gene
                ).tolist()
    ...
    for i in itertools.count():
        try:
            handle = Entrez.esearch(
                db="pmc", term=term, api_key=os.environ["API_KEY"], retstart=i * batch, retmax=batch
            )
...
def fetch_extract_gmt_from_oa_package(oa_package):
    ...
    with tempfile.NamedTemporaryFile(prefix="pathogl-", suffix="".join(PurePosixPath(oa_package).suffixes)) as tmp:
...
def main(
    ...
    if not gene_lookup_file.exists():
        ...
        ncbi_genes = ncbi_genes_fetch(
            organism="Archaea_Bacteria/Bacteria",
            filters=lambda ncbi: ((ncbi["type_of_gene"] == "protein-coding") & (ncbi["#tax_id"] == species)),
        )
        ...
        with output_file.open("a") as output_fh:
            with ThreadPool() as pool:
                for record, err, res in tqdm.tqdm(
                    pool.imap_unordered(task, ({"File": row["File"]} for _, row in oa_file_list.iterrows())),
