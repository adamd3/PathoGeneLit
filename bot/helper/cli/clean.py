    ...
    with input.open("r") as fr:
        with output.open("w") as fw:
            for line in filter(None, map(str.strip, fr)):
                ...
                geneset_mapped = unique(
                    [gene_mapped for gene in geneset for gene_mapped in (gene_lookup(gene),) if gene_mapped]
                )
                if len(geneset_mapped) >= 5 and len(geneset_mapped) < 1000 and len(term) < 300 and term not in terms:
                    terms.add(term)
                    print(
                        term,
                        "",
